...
__all__ = ['Utils', 'BinaryType', 'ValueType', 'ValueSource', 'Field', 'EnterParamField', 'EnterParamCollection',
           ...
           'Localization', 'LocalizationCulture', 'LocalizationScopes', 'TaskLocalizationItems', 'Extension',
           'set_paths_root']

PATHS_ROOT = ''

_mapping_type = Union[Dict['Attribute',
                           Union['Field', List['Field']]], 'Header']


def set_paths_root(path: str):
    """
...
class Utils:
    ...
    @classmethod
    def string(cls, path: str) -> str:
        """
        Gets string from given file. Use this to set SVG image properties

        :param path: path to file
        :return: string
        """
        try:
            with open(os.path.join(PATHS_ROOT, path), 'r') as file:
                return file.read()
        except:
            return ''
...
class _Checks:
    ...
    def validate_system_name(system_name: str):
        ...
        if not system_name[0].isalpha():
            raise Exception(
                f'system name must begin with alphabetic symbol, {system_name} given')
        for index, character in enumerate(system_name[1:], start=1):
            if not valid_symbol(character):
                raise Exception(
                    f'invalid symbol {character} in system name at {index}: {system_name}')
...
class EnterParamField(Field):
    ...
    def __init__(self, system_name, display_name, type, *, is_array: bool = False, required: bool = False,
                 ...
                 image: bool = False, default_value: Any = None, predefined_values: List[Any] = None,
                 value_sources: List[Union[ValueSource, str, 'Attribute']] = None, category: str = None,
                 description: str = None, system_provider_id: UUID = None):
        """
        ...
        :param image: True if field is a Base64 encoded image
        ...
        :param UUID system_provider_id: system enter parameter UUID
        """
        super().__init__(display_name, type, system_name=system_name)
        _Checks.check_arguments(arg_type=str, system_name=system_name)
        ...
        if self._is_array and type != ValueType.Integer and type != ValueType.Float and type != ValueType.String:
            raise Exception(
                'is_array option available only for integer, float and string enter parameters')
        ...
        if system_provider_id is not None:
            if not isinstance(system_provider_id, UUID):
                raise TypeError('system_provider_id must be an UUID')
            self._system_provider_id = str(system_provider_id)
    ...
    def to_json(self) -> Dict[str, Any]:
        ...
        try:
            result['value_sources'] = [s.to_json()
                                       for s in self._value_sources]
        except AttributeError:
            pass
        return result
...
class EnterParamCollection:
    ...
    def add_enter_param(self, system_name, display_name, type, *, is_array: bool = False, required: bool = False,
        ...
        :param image: is field representing Base64 encoded image
        :param default_value: default value, depending on ValueType
        ...
        self._append(EnterParamField(
            ...
            geo_json=geo_json, file_path=file_path, image=image,
            default_value=default_value, predefined_values=predefined_values, value_sources=value_sources,
            category=category, description=description, system_provider_id=system_provider_id))
    ...
    def _append(self, ep_field):
        ...
        if ep_field.system_name in self._fields:
            raise Exception(
                f'Enter parameter already exists: {ep_field.system_name}')
...
class Header(type):
    ...
    def __new__(mcs, name, bases, classdict):
        ...
        for field_name in classdict['__fields__']:
            ...
            if not isinstance(field, Field):
                raise TypeError(
                    f'Header fields must be instances of Field, {type(field)} given')
            if isinstance(field, EnterParamField):
                raise TypeError(
                    'Header fields must be instances of Field, not EnterParamField')
    ...
    def set_property(self, field_name: str, property: str, value: Any):
        ...
        _Checks.check_arguments(
            arg_type=str, field_name=field_name, property=property)
...
class Schema(type):
    ...
    def get_json(name: str, objects: list, links: list, scopes: List[str] = None,
        ...
        def set_entity_property(entity, key, value):
            properties = result['object_properties'] if type(
                entity) == Object else result['link_properties']
            e_name = entity.name
            if e_name:
        ...
        for entity in entities:
            ...
            # check for non-empty Image property
            if getattr(entity, 'Image', False) and entity.Image:
            ...
            # check for non-empty SvgImage property
            if getattr(entity, 'SvgImage', False) and entity.SvgImage:
                set_entity_property(entity, 'svg_image', entity.SvgImage)
            for attribute in entity.get_attributes():
                if attribute.svg_image:
                    set_attribute_property(
                        attribute, 'svg_image', attribute.svg_image)
        return result
            ...
            for link in links_copied:
                ...
                if link.end in graph_info:
                    if link.end == link.begin:
                        right_index = graph_info[link.end] = len(
                            list_of_objects)
    ...
    def __new__(mcs, name, bases, classdict):
        ...
        def check_entities(objects, links, header):
            field_system_names = {
                field.system_name: field for field in header.get_fields().values()}
                ...
                for attribute in entity.attributes:
                    for column in attribute.columns:
                        if column.system_name not in field_system_names:
                            raise Exception(
                                f'Field {column.system_name} not presented in header {header.display_name}')
        ...
        entities = {k: v for k, v in classdict.items()
                    if isinstance(v, SchemaEntity)}
        for base in bases:
            base_entities = {
                k: v for k, v in base.__dict__.items() if isinstance(v, SchemaEntity)}
        ...
        object_set = set([e for e in entities.values()
                          if isinstance(e, SchemaObject)])
        link_set = set([e for e in entities.values()
                        if isinstance(e, SchemaLink)])
        ...
        for base in bases:
            if header is None:
                ...
                if header != base.Header:
                    raise Exception(
                        'Differrent headers in base schemas are not allowed')
        if header is None:
    ...
    def to_json(self) -> Dict[str, Any]:
        self.__objects__, self.__links__ = Schema.process_graph(
            self.__objects__, self.__links__)
    ...
    def set_scopes(self, *scopes: str):
        ...
        """
        self.__scopes__ = list(
            set([scope for scope in scopes if scope in SchemaScope.values()]))
...
class SchemaCollection:
    ...
    def _add_schema(self, schema):
        ...
        if schema.name in [s.name for s in self._schemas]:
            raise Exception(
                f'Schema with this name already exists: {schema.name}')
...
class SchemaEntity:
    ...
    def __init__(self, entity_type: Union['Object', 'Link'], mapping: _mapping_type,
        ...
        if not (isinstance(entity_type, Object) or isinstance(entity_type, Link)):
            raise TypeError(
                'Schema entity type must be derivative of Object or Link')
        ...
        self._properties = {}
        _Checks.check_arguments(
            arg_type=str, condition_union_mode=condition_union_mode)
        ...
        _Checks.check_arguments(
            arg_type=bool, condition_ignore_case=condition_ignore_case)
        ...
        if isinstance(mapping, dict):
            for attr in mapping:
                if not isinstance(attr, Attribute):
                    raise TypeError(
                        f'Mapping keys must be of type Attribute, {type(attr)} given')
                if attr not in type_attributes:
                    raise Exception(
                        f'Invalid mapping: attribute {attr.name} not found in entity type')
    ...
    def _add_attribute(self, attribute):
        if attribute.name in [a.name for a in self._attributes]:
            raise Exception(
                f'Attribute with this name already exists: {attribute.name}')
...
class Object(type):
    ...
    def __new__(mcs, name, bases, classdict):
        ...
        for special_items in [idents, caps, ordered_attributes]:
            for item in special_items:
                if not isinstance(item, Attribute):
                    raise TypeError(
                        f'Object attributes must be instances of Attribute, {type(item)} given')
        ...
        for attr in ordered_attributes:
            ...
            if attr.name in known_attributes:
                raise Exception(
                    f'More than one attribute with name {attr.name}. Names must be unique')
            else:
        ...
        for char in '.$':
            if char in classdict['name']:
                raise Exception(
                    f'Object name can\'t contain character "{char}"')
    ...
    def schematic(cls, mapping: _mapping_type, conditions: List[Condition] = None,
                  condition_union_mode: str = UnionMode.And,
...
class Link(type):
    ...
    def __new__(mcs, name, bases, classdict):
        ...
        for attr in classdict['__attributes__']:
            if not isinstance(attr, Attribute):
                raise TypeError(
                    f'Link attributes must be instances of Attribute, {type(attr)} given')
    ...
    def between(cls, begin: 'SchemaObject', end: 'SchemaObject', mapping: _mapping_type,
        ...
        """
        return SchemaLink(cls, begin=begin, end=end, mapping=mapping, conditions=conditions, condition_union_mode=condition_union_mode, condition_ignore_case=condition_ignore_case)
...
class SchemaObject(SchemaEntity):
    ...
    def __init__(self, type: Object, mapping: _mapping_type, conditions: List[Condition] = None,
        ...
        """
        super().__init__(type, mapping, conditions,
                         condition_union_mode, condition_ignore_case)
...
class Attribute:
    ...
    def __init__(self, name: str, type: str, image: str = None, svg_image: str = None):
        """
        ...
        :param svg svg_image: (optional) string of attribute SVG image
        """
        ...
        for char in '.$':
            if char in name:
                raise Exception(
                    f'Attribute name can\'t contain character "{char}"')
        self._name = name
...
class Macro:
    ...
    def __init__(self, name: str, mapping_flags: List[str] = None,
                 ...
                 *,
                 drops: Union[Iterable[Union[EnterParamField, str]],
                              EnterParamField, str] = None,
                 drop_except: Union[Iterable[Union[EnterParamField,
                                                   str]], EnterParamField, str] = None,
                 category: str = ''):
        """
    ...
    def to_json(self) -> Dict[str, Any]:
        ...
        if self._drop_exceptions_exist:
            result['drop_except'] = list(
                self._drop_except) if self._drop_except else []
    ...
    def add_macros(self, *macros: Macro):
        ...
        for macro in macros:
            if macro.name in [m.name for m in self._macros]:
                raise Exception(
                    f'Macro with this name already exists: {macro.name}')
...
class Task(metaclass=abc.ABCMeta):
    ...
    def get_validate_function(self) -> str:
        """
        :return: validate function for task
        """
        return 'return {}'
    ...
    def get_extensions(self) -> Dict[str, 'Extension']:
        """
        :return: named extensions {'name1': extension1, 'name2': extension2}
        """
        pass
...
class ResultWriter:
    ...
    def __init__(self, output_file: str, headers: Union[Header, HeaderCollection]):
        ...
        if isinstance(headers, HeaderCollection):
            ...
            raise TypeError(
                'Headers must be HeaderCollection or single header')
    ...
    def write_line(self, values: Dict[Field, Any], header_class: Header = None):
        ...
        if header is None:
            raise Exception(
                'Cannot detect header for values. Check line length and column types')
    ...
    def write_dataframe(self, dataframe, header_class: Header = None):
        ...
        def flush(handler, df, column_order=None, string_columns=None):
            if string_columns:
                df[string_columns] = df[string_columns].applymap(
                    self._sanitize)  # remove line breaks from strings
            if column_order:
                df = df[column_order]
        ...
        def order_and_strings(header_cl, sn=True):
            if sn:
                ...
                str_cols = [
                    field.system_name for field in header_cl if field.type == ValueType.String]
            else:
                names_and_fields = header_cl.get_fields()
                field_order = list(names_and_fields)
                str_cols = [name for name, field in names_and_fields.items(
        ...
        if not header_class:
            ...
            if header:
                ...
                if header:
                    ...
                    raise Exception(
                        'Can\'t detect header or values not fit in header')
...
class RelativeDate:
    ...
    def __init__(self, reference_point: str, offset: datetime.timedelta):
        ...
        if reference_point not in ReferencePoint.values():
            raise Exception(
                'reference_point must be one of ReferencePoint enum values')
    ...
    def __new__(mcs, name, bases, classdict):
        ...
        for c_name in classdict['__cultures__']:
            ...
            if not isinstance(culture, LocalizationCulture):
                raise TypeError(
                    f'Localization cultures must be instances of LocalizationCulture, {type(culture)} given')
